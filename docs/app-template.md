...
6. **Pricing and account** live on each **product app** (`/pricing`, `/account`). The brand site has no `/pricing`. KVSHVL Pro is one subscription per account — app pages must say so and name the current ecosystem explicitly (Check Your Drawings, Coherence, and the entire ecosystem of software and knowledge).
